...
def get_data_for_period(period_choice):
    ...
    ticker = 'NVDA'  # Пример тикера акции (NVIDIA)
...
def chart(period_choice):
    ...
    ax.set_xlabel('Date', color='black')  # Цвет оси абсцисс
    ax.set_ylabel('Closing Price', color='black')  # Цвет оси ординат
    ax.tick_params(axis='x', colors='black')  # Цвет делений оси абсцисс
    ax.tick_params(axis='y', colors='black')  # Цвет делений оси ординат
    ax.legend()
